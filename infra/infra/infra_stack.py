...
class InfraStackRizwan(cdk.Stack):
    ...
    def __init__(self, scope: cdk.Construct, construct_id: str, **kwargs) -> None:
        ...
        for i in URLS:
            ...
            availability_metric=cloudwatch.Metric( metric_name=constants.URL_MONITOR_METRIC_AVAILABILITY, namespace=constants.URL_MONITOR_NAMESPACE, dimensions=dimensions, label='Availability_metric_of _{}'.format(i))
            # Defining latency metric
            latency_metric=cloudwatch.Metric( metric_name=constants.URL_MONITOR_METRIC_LATENCY, namespace=constants.URL_MONITOR_NAMESPACE, dimensions=dimensions, label='Latency_metric_of_{}'.format(i))
            ...
            , evaluation_periods=1, threshold=constants.THRESHOLD_OF_LATENCY, alarm_name='Latency_Alarm_of__{}'.format(i), comparison_operator=cloudwatch.ComparisonOperator.GREATER_THAN_THRESHOLD, datapoints_to_alarm=1)
            ...
            , evaluation_periods=1, threshold=constants.THRESHOLD_OF_AVAILABILITY,alarm_name='Availability_Alarm_of__{}'.format(i), comparison_operator=cloudwatch.ComparisonOperator.LESS_THAN_THRESHOLD, datapoints_to_alarm=1)
